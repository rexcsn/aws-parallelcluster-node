...
class OptimalNodeCountTests(unittest.TestCase):
    ...
    def test_each_node_at_capacity(self):
        nodes = utils.get_optimal_nodes([1, 5, 3], [8, 40, 24], instance_properties["slots"])
        expected = 9
        self.assertEqual(nodes, expected)

    def test_slots_requested_greater_than_available(self):
        nodes = utils.get_optimal_nodes([1], [9], instance_properties["slots"])
        expected = 0
        self.assertEqual(nodes, expected)

    def test_each_node_half_capacity(self):
        nodes = utils.get_optimal_nodes([1, 5, 3], [4, 20, 12], instance_properties["slots"])
        expected = 5
        self.assertEqual(nodes, expected)

    def test_each_node_one_vcpu_except_max(self):
        nodes = utils.get_optimal_nodes([1, 5, 3], [1, 40, 1], instance_properties["slots"])
        expected = 8
        self.assertEqual(nodes, expected)

    def test_each_node_partial_capacity(self):
        nodes = utils.get_optimal_nodes([1, 5, 3, 2], [6, 35, 1, 1], instance_properties["slots"])
        expected = 6
        self.assertEqual(nodes, expected)
