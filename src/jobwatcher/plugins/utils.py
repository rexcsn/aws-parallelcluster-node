...
def get_optimal_nodes(nodes_requested, slots_requested, resources_per_node):
    """
    ...
    for slots, num_of_nodes in zip(slots_requested, nodes_requested):
        ...
        if slots_required_per_node > resources_per_node:
            log.warning(
                "Slots required per node (%d) is greater than resources available on single node (%d), skipping job...",
        ...
        for _ in range(num_of_nodes):
            ...
            slots_remaining_per_node.append(resources_per_node - slots_required_per_node)
